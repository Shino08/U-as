...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-4 sm:px-8 pt-4 pb-2 transition-all duration-300 pointer-events-none">
      <nav
        className={cn(
          "max-w-7xl mx-auto rounded-full px-6 h-16 flex items-center justify-between shadow-2xl transition-all duration-300 pointer-events-auto backdrop-blur-xl border",
          !scrolled
      ...
      >
        <Link to="/" className="flex items-center gap-3 group focus:outline-none">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className={cn("font-display italic text-2xl font-semibold tracking-tight", !scrolled || theme === "dark" ? "text-white" : "text-[#1d0f1c]")}>
              Lumière
            </span>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className={cn("lg:hidden p-2 rounded-xl transition-colors cursor-pointer", !scrolled ? "text-white" : theme === "dark" ? "text-[#f29ebb]" : "text-[#b33261]")}
      ...
      {mobileMenuOpen && (
        <div
          className={cn(
            "lg:hidden mt-2 max-w-7xl mx-auto rounded-3xl p-6 space-y-4 shadow-2xl border animate-in fade-in slide-in-from-top-3 pointer-events-auto backdrop-blur-2xl",
        ...
        >
          {!isDashboard ? (
            <>
              {NAV_LINKS.map((item) => (
                <a key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className="block text-sm font-semibold py-1.5 hover:text-[#d9487d]">
                  {item.label}
                </a>
              ))}
              <a
            ...
            <Link to="/" onClick={() => setMobileMenuOpen(false)} className="block text-sm font-semibold py-2 text-[#d9487d]">
              ← Volver a la Landing
            </Link>
          )}
